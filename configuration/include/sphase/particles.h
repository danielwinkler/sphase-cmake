...
{
public:
    explicit ParticleHelper(const Configuration<number>& configuration, bool grid_align);
    ~ParticleHelper();

    Particles2<number> *getParticles();
    Particles2<number> *createDigester(number radius, number centerx, number centery, number angletop, number angledown);
