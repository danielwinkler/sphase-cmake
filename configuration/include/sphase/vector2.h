...
public:
    ...
    //2D tensor product
    static inline
    Matrix2 OProduct(const Vector2<number> &v1, const Vector2<number> &v2) {
        return Matrix2(v1.x*v2.x,
                       v1.x*v2.y,
                       v1.y*v2.x,
                       v1.y*v2.y);
    }
    ...
    Matrix2 Transpose() const {
        const number xy = m12;
        return Matrix2(m11,m21,xy,m22);
    }

    inline
    Matrix2 operator/(number alpha) const {
        return Matrix2(m11/alpha, m12/alpha, m21/alpha, m22/alpha);
    }
    ...
    inline
    Matrix2 operator-(const Matrix2 &m) const {
        return Matrix2(m11-m.m11, m12-m.m12, m21-m.m21, m22-m.m22);
    }
